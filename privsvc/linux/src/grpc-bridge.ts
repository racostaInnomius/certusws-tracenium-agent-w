...
//
// Direct port of privsvc/macos/src/grpc-bridge.ts. Every Patch
// (A through M) is preserved verbatim — they are transport-level
// fixes that apply identically on Linux. Only three things differ
// from the macOS source:
//
//   1. Header comment block (this one) — orienting future readers.
//   2. Proto path resolver — falls back through several candidates so
//      a developer can run the daemon directly from a checkout
//      without going through the .deb/.rpm build pipeline. Production
//      installs hit the first candidate (next-to-dist) which mirrors
//      macOS exactly.
//   3. References in comments to "launchd KeepAlive=true" → adjusted
//      to "systemd Restart=always". Functionally identical
//      (init system relaunches the daemon on exit) but the wording
//      should match the platform an operator is reading.
//
// Everything else — including the historical macOS-incident commentary
// in the Patch A–M blocks — is left intact. The bug we fixed by adding
// each patch was not OS-specific (it was a @grpc/grpc-js transport
// behaviour) and the comments document WHY the code looks the way it
// does. Stripping that context to make the file feel "Linux-native"
// would just relitigate the same bug six months from now.
...
//      installs / debugging.
//   2. <bundle>/proto/controlplane.proto — production layout. The
//      Phase-4 packaging (.deb/.rpm) puts the proto at this path
//      relative to dist/. Mirrors the macOS build script.
//   3. <repo-root>/proto/controlplane.proto — dev run from a
//      checkout where the build script hasn't been run yet.
//   4. <cwd>/proto/controlplane.proto — for `node dist/index.js`
//      invocations from the repo root.
//
// We resolve at module load (cheap, sync) rather than lazily so a
// missing proto fails the daemon at boot, with a clear message,
// instead of at first grpc.connect call buried in a stack trace.
function resolveProtoPath(): string {
  const candidates = [
    process.env.TRACENIUM_PROTO_PATH,
    path.resolve(__dirname, "../proto/controlplane.proto"),
    path.resolve(__dirname, "../../proto/controlplane.proto"),
    path.resolve(__dirname, "../../../proto/controlplane.proto"),
    path.resolve(process.cwd(), "proto/controlplane.proto"),
  ...
  throw new Error(
    `controlplane.proto not found. Tried: ${candidates.join(", ")}. ` +
    `Set TRACENIUM_PROTO_PATH or run from a layout where the proto is ` +
    `next to the dist/ directory.`
  );
}
...
//
// The Linux privsvc inherits this same `@grpc/grpc-js` runtime
// behaviour, so the same bug would happen here if we re-introduced
// the keepalive options. Keep this empty.
//
...
//
// Net behaviour: every platform now matches Windows on the wire — no
// client pings, server-driven liveness, app watchdog as belt-and-
// braces. The 30-s detection latency we lose by removing client pings
// is well within tolerance: an idle bidi that's actually dead will
// be caught by either (1) the next server ping (≤ 60 s) failing, or
...
// will fix it — none of the higher layers will, because systemd thinks
...
//     `process.exit(1)`. systemd's `Restart=always` in
//     tracenium-privsvc.service relaunches us immediately, which gets
//     us a fresh process, fresh gRPC client, fresh TLS handshake — the
...
function tripBreaker(reason: string, details: Record<string, any>) {
  ...
  // stdout still occasionally block-buffered we want the systemd
  // journal capture AND the IPC pipe BOTH to drain before we exit.
...
function tickBreaker() {
  ...
  // systemd hands us a fresh process with a fresh socket.
  //
...
// ── Dead-stream watchdog (parity with Windows GrpcBridge.cs) ─────────
//
// The Windows bridge has had this since day one and it's been catching
// zombies in production. Mirroring the same shape here so Linux doesn't
...
async function startConnection(params: Record<string, any>, pushSink: PushSink) {
  ...
  try {
    ...
    // sit zombie for hours otherwise; same risk on Linux laptops post-
    // suspend. Mirrors what the Windows bridge does.
    startWatchdog();
...
export async function handleFactsSend(req: PrivSvcRequest): Promise<PrivSvcResponse> {
  try {
    ...
    //
    // The Facts proto has these relevant fields:
    //   string eventId = 1;
    //   bytes  payloadJson = 2;
    //   string deviceId = 3;
    //   string namespace = 4;
    ...
    //
    // Windows and macOS PrivSvc both forward these fields. Linux must
    // do the same — a bug here would manifest as silent ACK rejection
    // post-IPC and a frozen device row on the dashboard.
