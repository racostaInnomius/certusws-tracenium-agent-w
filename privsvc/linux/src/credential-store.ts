// privsvc/linux/src/credential-store.ts
//
...
//
// STORAGE CHOICE — why a file and not libsecret
// The documented preference was libsecret, but libsecret talks to a
// gnome-keyring / kwallet daemon over the *session* D-Bus. PrivSvc runs as a
// system daemon with no session and no unlocked keyring, so on a headless
// server — which is exactly what a gateway host is — libsecret is either absent
// or permanently locked. Rather than ship a path that silently fails in the
// deployment we actually target, the credential is stored AES-256-GCM encrypted
// under a per-install master key, both files root-owned and 0600. On a Linux
// system daemon that file mode IS the security boundary: anything that can read
// the master key can already read the enrollment private key next to it.
