...
public static class SecurityCompliance
{
    ...
    // Platform integrity — TPM y UEFI Secure Boot. La forma (y por qué un
    // fallo de lectura es "unknown" y no "ausente") vive en
    // PlatformIntegrityShape.cs, donde se prueba.
    private static object GetTpmStatus()
    {
        try
        {
            return PlatformIntegrityShape.ParseTpm(RunPs(PlatformIntegrityShape.TpmScript));
        }
        catch
        {
            return PlatformIntegrityShape.Unknown();
        }
    }
    ...
    private static object GetSecureBootStatus()
    {
        try
        {
            return PlatformIntegrityShape.ParseSecureBoot(RunPs(PlatformIntegrityShape.SecureBootScript));
        }
        catch
        {
            return PlatformIntegrityShape.Unknown();
