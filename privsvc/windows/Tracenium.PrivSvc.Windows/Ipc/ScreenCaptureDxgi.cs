...
internal static class ScreenCaptureDxgi
{
    ...
    // ⚠️ Los structs de DXGI viven en DxgiInteropTypes.cs.
    //
    // No es una separacion estetica: este fichero usa System.Drawing.Imaging
    // y por eso no se puede compilar en el proyecto de pruebas, que es
    // `net8.0` a secas para poder correr fuera de Windows. Alli se mide su
    // disposicion en memoria con `Marshal.SizeOf` — un tamano equivocado en
    // `DXGI_OUTDUPL_FRAME_INFO` no da error ni excepcion: DXGI escribe mas
    // alla del struct y el proceso se cae despues, en otro sitio.
