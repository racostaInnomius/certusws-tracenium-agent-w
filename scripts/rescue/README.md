...
scp scripts/rescue/tracenium-rescue-macos.sh \
    build/pkg-out/Tracenium-Agent-1.1.19-arm64.pkg \
    user@mac-host:/tmp/

# SSH in:
ssh user@mac-host
sudo /bin/sh /tmp/tracenium-rescue-macos.sh /tmp/Tracenium-Agent-1.1.19-arm64.pkg
```
...
scp build/win-msi/x64/Tracenium-Agent-1.1.19-x64.msi USER@HOST:C:/Temp/

# RDP in, open PowerShell as Administrator:
cd C:\Temp
Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass
.\tracenium-rescue-windows.ps1 -MsiPath .\Tracenium-Agent-1.1.19-x64.msi
```
...
3. **Manual / IT ticket**:
   ...
   - Document the expected before/after state ("agent version goes from
     1.1.14 → 1.1.19 in the portal within 2 minutes")
...
If a device is at 1.1.19+ and a future release lands (1.1.19, etc.),
