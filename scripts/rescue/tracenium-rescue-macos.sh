...
#
# Usage:
#   sudo ./tracenium-rescue-macos.sh /path/to/Tracenium-Agent-1.1.15-arm64.pkg
#
# What it does:
...
PKG="${1:-}"
if [ -z "$PKG" ] || [ ! -f "$PKG" ]; then
  echo "Usage: sudo $0 /path/to/Tracenium-Agent-1.1.15-arm64.pkg" >&2
  exit 1
fi
...
echo ""
echo "✓ Rescue complete. Device should report 1.1.15 to backend within 1-2 min."
echo "  Verify: should appear at version 1.1.15 in the portal."
