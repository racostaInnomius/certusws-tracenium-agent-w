...
function getSoftwareBaselineDbPath() {
  return import_path.default.join(agentDataDir(), "agent.db");
}
...
var init_privsvc_client_windows = __esm({
  "src/priv/privsvc-client-windows.ts"() {
    ...
    PrivSvcClient = class extends import_events.EventEmitter {
      socket = null;
      buffer = "";
      connecting = null;
      ...
      onData(data) {
        ...
        while ((newlineIndex = this.buffer.indexOf("\n")) >= 0) {
          ...
          const id = msg?.id;
          const isResponse = typeof id === "string" && (Object.prototype.hasOwnProperty.call(msg, "ok") || Object.prototype.hasOwnProperty.call(msg, "error"));
          if (isResponse) {
            if (!id) {
          ...
          if (isPush) {
            if (this.hasPushListeners()) {
              ...
              if (this.earlyPushQueue.length < 1e3) {
...
function getTimeoutForMethod2(method) {
  switch (method) {
    case "grpc.connect":
      return 6e4;
    case "grpc.facts.send":
...
var init_privsvc_client_macos = __esm({
  "src/priv/privsvc-client-macos.ts"() {
    ...
    CONNECT_TIMEOUT_MS2 = 8e3;
    MAX_PENDING2 = 500;
    MAX_BUFFER_CHARS2 = 2 * 1024 * 1024;
    PrivSvcClient2 = class extends import_events2.EventEmitter {
      socket = null;
      buffer = "";
      connecting = null;
      pending = /* @__PURE__ */ new Map();
      ...
      onData(data) {
        ...
        this.buffer += chunk;
        if (this.buffer.length > MAX_BUFFER_CHARS2) {
          this.buffer = "";
...
function getTimeoutForMethod3(method) {
  switch (method) {
    case "grpc.connect":
      return 6e4;
    case "grpc.facts.send":
...
var init_privsvc_client_linux = __esm({
  "src/priv/privsvc-client-linux.ts"() {
    ...
    CONNECT_TIMEOUT_MS3 = 8e3;
    MAX_PENDING3 = 500;
    MAX_BUFFER_CHARS3 = 2 * 1024 * 1024;
    PrivSvcClient3 = class extends import_events3.EventEmitter {
      socket = null;
      buffer = "";
      connecting = null;
      pending = /* @__PURE__ */ new Map();
      ...
      onData(data) {
        ...
        this.emit("debug", { stage: "raw_chunk" });
        this.buffer += chunk;
        if (this.buffer.length > MAX_BUFFER_CHARS3) {
          this.buffer = "";
          this.onSocketError(new Error("PrivSvc buffer overflow"));
          try {
            this.socket?.destroy();
  ...
  "node_modules/systeminformation/lib/util.js"(exports2) {
    ...
    function getCodepage() {
      if (_windows) {
        if (!codepage) {
          try {
            const stdout = execSync3("chcp", execOptsWin);
      ...
      if (_linux || _darwin || _freebsd || _openbsd || _netbsd) {
        if (!codepage) {
          try {
            const stdout = execSync3("echo $LANG", execOptsLinux);
            const lines = stdout.toString().split("\r\n");
            const parts = lines[0].split(".");
    ...
    function smartMonToolsInstalled() {
      ...
      if (_windows) {
        try {
          const pathArray = execSync3("WHERE smartctl 2>nul", execOptsWin).toString().split("\r\n");
      ...
      if (_linux || _darwin || _freebsd || _openbsd || _netbsd) {
        try {
          const pathArray = execSync3("which smartctl 2>/dev/null", execOptsLinux).toString().split("\r\n");
    ...
    function linuxVersion() {
      ...
      if (_linux) {
        try {
          result2 = execSync3("uname -v", execOptsLinux).toString();
        } catch {
          result2 = "";
        }
      }
    ...
    function checkWebsite(url, timeout = 5e3) {
      const http2 = url.startsWith("https:") || url.indexOf(":443/") > 0 || url.indexOf(":8443/") > 0 ? require("https") : require("http");
      const t = Date.now();
      return new Promise((resolve) => {
        ...
        process.nextTick(() => {
          ...
          if (_linux || _freebsd || _openbsd || _netbsd) {
            ...
            echo -n "sys_vendor: "; cat /sys/devices/virtual/dmi/id/sys_vendor 2>/dev/null; echo;`;
              try {
                lines = execSync3(cmd, util.execOptsLinux).toString().split("\n");
              ...
              if (!result2.virtual) {
                try {
                  const disksById = execSync3("ls -1 /dev/disk/by-id/ 2>/dev/null; pciconf -lv  2>/dev/null", util.execOptsLinux).toString();
              ...
              if (_freebsd || _openbsd || _netbsd) {
                try {
                  const lines2 = execSync3("sysctl -i kern.hostuuid kern.hostid hw.model", util.execOptsLinux).toString().split("\n");
                  if (!result2.uuid) {
              ...
              if ((_freebsd || _openbsd || _netbsd) && !result2.virtualHost) {
                try {
                  const procInfo = execSync3("dmidecode -t 4", util.execOptsLinux);
              ...
              try {
                const stdout2 = execSync3('dmesg 2>/dev/null | grep -iE "virtual|hypervisor" | grep -iE "vmware|qemu|kvm|xen" | grep -viE "Nested Virtualization|/virtual/"');
        ...
        process.nextTick(() => {
          ...
          if (_linux || _freebsd || _openbsd || _netbsd) {
            ...
            echo -n "bios_version: "; cat /sys/devices/virtual/dmi/id/bios_version 2>/dev/null; echo;`;
              try {
                lines = execSync3(cmd2, util.execOptsLinux).toString().split("\n");
  ...
  "node_modules/systeminformation/lib/cpu.js"(exports2) {
    ...
    function getCpuCurrentSpeedSync() {
      ...
      if (cpus && cpus.length && Object.prototype.hasOwnProperty.call(cpus[0], "speed")) {
        ...
        try {
          const speedStrings = execSync3('cat /proc/cpuinfo | grep "cpu MHz" | cut -d " " -f 3', util.execOptsLinux).toString().split("\n").filter((line) => line.length > 0);
        ...
        process.nextTick(() => {
          ...
          if (_linux) {
            try {
              ...
              const parts = execSync3(cmd2, util.execOptsLinux).toString().split("-----\n");
              if (parts.length === 2) {
      ...
      return new Promise((resolve) => {
        process.nextTick(() => {
          ...
          if (now >= 200) {
            ...
            if (_linux) {
              try {
                const lines = execSync3("cat /proc/stat 2>/dev/null | grep cpu", util.execOptsLinux).toString().split("\n");
                if (lines.length > 1) {
                  lines.shift();
      ...
      return new Promise((resolve) => {
        process.nextTick(() => {
          ...
          if (_darwin) {
            let pageSize = 4096;
            try {
              let sysPpageSize = util.toInt(execSync3("sysctl -n vm.pagesize").toString());
            ...
            exec2(
              ...
              (error, stdout) => {
                ...
                if (!result2.length) {
                  ...
                  try {
                    let stdout2 = execSync3("cat /proc/cpuinfo 2>/dev/null", util.execOptsLinux);
                    let lines = stdout2.toString().split("\n");
                    ...
                    if (util.isRaspberry(lines)) {
                      ...
                      stdout2 = execSync3("vcgencmd get_config sdram_freq 2>/dev/null", util.execOptsLinux);
                      lines = stdout2.toString().split("\n");
                      let freq = parseInt(util.getValue(lines, "sdram_freq", "=", true), 10) || 0;
                      if (freq) {
                        result2[0].clockSpeed = freq;
                      }
                      stdout2 = execSync3("vcgencmd measure_volts sdram_p 2>/dev/null", util.execOptsLinux);
                      lines = stdout2.toString().split("\n");
  ...
  "node_modules/systeminformation/lib/graphics.js"(exports2) {
    ...
    function graphics(callback) {
      ...
      function parseLinesLinuxControllers(lines) {
        ...
        let pciIDs = [];
        try {
          pciIDs = execSync3('export LC_ALL=C; dmidecode -t 9 2>/dev/null; unset LC_ALL | grep "Bus Address: "', util.execOptsLinux).toString().split("\n");
      ...
      function nvidiaSmi(options) {
        ...
        if (nvidiaSmiExe) {
          ...
          try {
            ...
            const res = execSync3(sanitized, options).toString();
            return res;
          } catch {
            util.noop();
          }
        ...
        process.nextTick(() => {
          ...
          if (_linux || _freebsd || _openbsd || _netbsd || _darwin) {
            ...
            if (_darwin) {
              cmd = "df -kP";
              try {
                macOsDisks = execSync3("diskutil list").toString().split("\n").filter((line) => {
                  return !line.startsWith("/") && line.indexOf(":") > 0;
                });
                execSync3("mount").toString().split("\n").filter((line) => {
            ...
            if (_linux) {
              try {
                ...
                execSync3("cat /proc/mounts 2>/dev/null", util.execOptsLinux).toString().split("\n").filter((line) => {
                  return line.startsWith("/");
                }).forEach((line) => {
      ...
      try {
        data.forEach((element) => {
          if (element.type.startsWith("raid")) {
            const lines = execSync3(`mdadm --export --detail /dev/${element.name}`, util.execOptsLinux).toString().split("\n");
              ...
              if (!error) {
                try {
                  ...
                  devices.forEach((device) => {
                    ...
                    try {
                      mediumType = execSync3("cat /sys/block/" + logical + "/queue/rotational 2>/dev/null", util.execOptsLinux).toString().split("\n")[0];
                    } catch {
                      util.noop();
                    }
          ...
          if (_windows) {
            try {
              ...
              if (util.smartMonToolsInstalled()) {
                try {
                  const smartDev = JSON.parse(execSync3("smartctl --scan -j").toString());
...
var require_network = __commonJS({
  "node_modules/systeminformation/lib/network.js"(exports2) {
    ...
    function getDefaultNetworkInterface() {
      ...
      try {
        ...
        if (_linux) {
          ...
          const result2 = execSync3(cmd, util.execOptsLinux);
    ...
    function getMacAddresses() {
      ...
      if (_linux || _freebsd || _openbsd || _netbsd) {
        if (typeof pathToIp === "undefined") {
          try {
            const lines = execSync3("which ip", util.execOptsLinux).toString().split("\n");
        ...
        try {
          ...
          const res = execSync3(cmd, util.execOptsLinux);
    ...
    function getWindowsDNSsuffixes() {
      ...
      try {
        const ipconfig = execSync3("ipconfig /all", util.execOptsWin);
    ...
    function getWindowsWiredProfilesInformation() {
      try {
        const result2 = execSync3("netsh lan show profiles", util.execOptsWin);
    ...
    function getWindowsWirelessIfaceSSID(interfaceName) {
      try {
        const result2 = execSync3(`netsh wlan show  interface name="${interfaceName}" | findstr "SSID"`, util.execOptsWin);
    ...
    function getDarwinNics() {
      ...
      try {
        const lines = execSync3(cmd, { maxBuffer: 1024 * 102400 }).toString().split("\n");
    ...
    function checkLinuxDCHPInterfaces(file) {
      ...
      try {
        ...
        const lines = execSync3(cmd, util.execOptsLinux).toString().split("\n");
        lines.forEach((line) => {
...
var require_wifi = __commonJS({
  "node_modules/systeminformation/lib/wifi.js"(exports2) {
    ...
    function ifaceListLinux() {
      ...
      try {
        const all = execSync3(cmd, util.execOptsLinux).toString().split("\n").map((line) => line.trim()).join("\n");
        ...
        try {
          const all = execSync3("nmcli -t -f general,wifi-properties,wired-properties,interface-flags,capabilities,nsp device show 2>/dev/null", util.execOptsLinux).toString();
          const parts = all.split("\n\n");
          let i = 1;
          parts.forEach((ifaceDetails) => {
            const lines = ifaceDetails.split("\n");
    ...
    function getWifiNetworkListNmi() {
      ...
      try {
        const stdout = execSync3(cmd, util.execOptsLinux);
    ...
    function getWifiNetworkListIw(iface) {
      const result2 = [];
      try {
        let iwlistParts = execSync3(`export LC_ALL=C; iwlist ${iface} scan 2>&1; unset LC_ALL`, util.execOptsLinux).toString().split("        Cell ");
      ...
      return new Promise((resolve) => {
        process.nextTick(() => {
          ...
          if (_linux) {
            ...
            if (result2.length === 0) {
              try {
                const iwconfigParts = execSync3("export LC_ALL=C; iwconfig 2>/dev/null; unset LC_ALL", util.execOptsLinux).toString().split("\n\n");
                let iface = "";
          ...
          if (srv) {
            ...
            if (_linux || _freebsd || _openbsd || _netbsd || _darwin) {
              if ((_linux || _freebsd || _openbsd || _netbsd) && srvString === "*") {
                try {
                  const tmpsrv = execSync3("systemctl --all --type=service --no-legend 2> /dev/null", util.execOptsLinux).toString().split("\n");
                  srvs = [];
                  ...
                  try {
                    srvString = "";
                    const tmpsrv = execSync3("service --status-all 2> /dev/null", util.execOptsLinux).toString().split("\n");
                    for (const s2 of tmpsrv) {
                    ...
                    try {
                      const srvStr = execSync3("ls /etc/init.d/ -m 2> /dev/null", util.execOptsLinux).toString().split("\n").join("");
                      srvString = "";
                      if (srvStr) {
...
var require_dockerSocket = __commonJS({
  "node_modules/systeminformation/lib/dockerSocket.js"(exports2, module2) {
    ...
    var DockerSocket = class {
      getInfo(callback) {
        try {
          let socket = net4.createConnection({ path: socketPath });
          let alldata = "";
          let data;
          socket.on("connect", () => {
...
var require_audio = __commonJS({
  "node_modules/systeminformation/lib/audio.js"(exports2) {
    ...
    function getLinuxAudioPci() {
      ...
      const result2 = [];
      try {
        const parts = execSync3(cmd, util.execOptsLinux).toString().split("\n\n");
      ...
      return new Promise((resolve) => {
        process.nextTick(() => {
          ...
          if (_linux) {
            ...
            try {
              const hdicon = execSync3("hcitool con", util.execOptsLinux).toString().toLowerCase();
              for (let i = 0; i < result2.length; i++) {
...
function computeSoftwareDelta(current, previous) {
  ...
  const sortById = (a, b) => (a.installId || "").localeCompare(b.installId || "");
  added.sort(sortById);
...
function isAppUpdated(prev, curr) {
  ...
  if (prevVersion === currVersion && prevPublisher === currPublisher && prevLocation === currLocation && prevPkg === currPkg) {
    return false;
  }
  return true;
}
...
function toBaselineOps(delta) {
  const upserts = [
    ...delta.added,
    ...delta.updated
  ];
  const deletes = delta.removed.map((a) => a.installId).filter((id) => Boolean(id));
...
function getDb() {
  ...
  dbInstance = new import_better_sqlite3.default(DB_PATH);
...
function deleteSoftwareByIds(installIds) {
...
async function collectWindowsDeviceAndHardware() {
  const [osInfo, system, cpu, mem, diskLayout, fsSize] = await Promise.all([
    ...
    import_systeminformation.default.system(),
  ...
  return {
    hardware: {
      static: {
        system: {
          manufacturer: system.manufacturer || void 0,
          model: system.model || void 0,
          version: system.version || void 0,
          serial: system.serial || void 0,
          uuid: system.uuid || void 0,
          virtual: Boolean(system.virtual)
        },
        cpu: {
          manufacturer: cpu.manufacturer || void 0,
          brand: cpu.brand || void 0,
          cores: cpu.cores || void 0,
          physicalCores: cpu.physicalCores || void 0
        },
        memLayout: void 0,
        diskLayout: diskLayout || void 0
      },
      runtime: {
        mem: mem ? { total: mem.total } : void 0,
        fsSize: fsSize || void 0
      }
    }
  };
}
async function collectWindowsSecurity(ctx) {
  const unknown = {
...
async function collectWindowsSoftwareInventory(ctx) {
  const resp = await ctx.priv.call({
    v: 1,
    id: `inv_${Date.now()}`,
  ...
  "src/plugins/amp/providers/windows.ts"() {
    ...
    windowsProvider = {
      async collect(ctx) {
        ...
        try {
          const result2 = await collectWindowsSoftwareInventory(ctx);
          const apps = result2.apps;
          if (!apps || apps.length === 0) {
            console.warn("[AGENT] EMPTY INVENTORY RECEIVED \u2014 CLEARING BASELINE");
            const previous2 = loadSoftwareBaseline() ?? [];
            const ids = previous2.map((x) => x.installId).filter((id) => Boolean(id));
          ...
          apps.sort(
            (a, b) => (a.installId ?? "").localeCompare(b.installId ?? "")
          );
          ...
          if (isFirstRun) {
            ...
            if (deltaResult.hasChanges) {
              ...
              software = {
                count: deltaResult.currentCount,
                items: void 0,
                // avoid affecting baseline hash / payload
                delta: null,
                hasChanges: false
              };
              return {
...
async function collectMacHardware() {
  const [system, cpu, mem, diskLayout, fsSize] = await Promise.all([
    import_systeminformation2.default.system(),
    import_systeminformation2.default.cpu(),
    import_systeminformation2.default.mem(),
    import_systeminformation2.default.diskLayout().catch(() => []),
    import_systeminformation2.default.fsSize().catch(() => [])
  ]);
  return {
    static: {
      system: {
...
async function collectMacSecurity(ctx) {
  const unknown = {
    bitlocker: { status: "unknown" },
    defender: { status: "unknown" },
    firewall: { status: "unknown" }
        ...
        try {
          ...
          if (!apps || apps.length === 0) {
            ...
            if (previous2.length > 0) {
              deleteSoftwareByIds(previous2.map((x) => x.installId).filter(Boolean));
            }
            return {
              hardware,
              security,
...
async function collectLinuxHardware() {
  const [system, cpu, mem, diskLayout, fsSize] = await Promise.all([
    ...
    import_systeminformation3.default.cpu(),
    import_systeminformation3.default.mem(),
    import_systeminformation3.default.diskLayout().catch(() => []),
    import_systeminformation3.default.fsSize().catch(() => [])
  ]);
  return {
    static: {
      system: {
  ...
  "src/plugins/amp/providers/linux.ts"() {
    ...
    linuxProvider = {
      async collect(ctx) {
        ...
        try {
          const apps = await collectLinuxSoftware();
          if (!apps || apps.length === 0) {
...
function updateUpdateState(patch) {
  ...
  if (patch.lastError) {
    next.status = "failed";
    next.lastCompletedAtUtc = (/* @__PURE__ */ new Date()).toISOString();
    next.updateInProgress = false;
  }
  if (patch.lastError) {
    next.installStartedAtUtc = void 0;
  }
  saveUpdateState(next);
  return next;
}
function markUpdateSucceeded(version) {
...
function markUpdateFailed(error) {
  return updateUpdateState({
    updateInProgress: false,
    status: "failed",
    lastError: error,
...
function getAuthToken(ctx) {
  return ctx.enrollment?.accessToken || ctx.enrollment?.bootstrap?.accessToken || ctx.enrollment?.token;
}
...
async function fetchAgentMetadata(ctx) {
...
function checkForAvailableUpdate(currentVersion, metadata) {
  ...
  if (metadata.minSupportedVersion && looksLikeSemver(metadata.minSupportedVersion) && compareSemver(currentVersion, metadata.minSupportedVersion) < 0) {
    return {
      available: true,
      currentVersion,
      latestVersion,
...
async function fetchWindowsMsiDownloadUrl(ctx, version = "latest") {
  const base = getApiBaseUrl(ctx);
  const arch = getArch();
  const url = `${base}/api/v1/binaries/agent?platform=windows&arch=${encodeURIComponent(arch)}&format=msi&version=${encodeURIComponent(version)}`;
...
function shouldCheckNow(intervalMs) {
...
async function runUpdateTask(ctx, opts) {
  const logger2 = opts?.logger;
  ...
  const state = reconcilePendingUpdate(currentVersion, logger2);
  ...
  if (freshState.updateInProgress) {
    ...
    if (nowMs() - lastAttempt < 10 * 60 * 1e3) {
      logger2?.warn?.("[update] update already in progress, skipping");
      return;
    }
    logger2?.warn?.("[update] stale updateInProgress detected, marking failed");
  ...
  try {
    let resolveArch2 = function() {
      const envArch = process.env.TRACENIUM_ARCH;
      if (envArch === "arm64" || envArch === "x64") {
        return envArch;
      }
      if (process.platform === "win32") {
        const arch2 = process.env.PROCESSOR_ARCHITECTURE;
        const wow64 = process.env.PROCESSOR_ARCHITEW6432;
        if (arch2 === "ARM64" || wow64 === "ARM64") {
          return "arm64";
        }
      }
      return process.arch === "arm64" ? "arm64" : "x64";
    };
    var resolveArch = resolveArch2;
    logger2?.info?.("[update] starting metadata fetch", {
      currentVersion,
      platform: ctx.agent?.platform || process.platform,
      force
    });
    const metadata = await fetchAgentMetadata(ctx);
    const result2 = checkForAvailableUpdate(currentVersion, metadata);
    const effectiveVersion = targetVersion || result2.latestVersion;
    const latestVersion = result2.latestVersion;
    freshState = loadUpdateState();
    if (freshState.lastAttemptedVersion && latestVersion && freshState.lastAttemptedVersion === latestVersion && freshState.lastSuccessVersion === latestVersion) {
      logger2?.info?.("[update] already attempted AND completed latest version, skipping", {
        currentVersion,
        latestVersion,
    ...
    logger2?.info?.("[update] metadata evaluated");
    if (!result2.available) {
      logger2?.info?.("[update] no update available", {
        currentVersion,
    ...
    if (!expectedHash) {
      logger2?.warn?.("[update] missing expected hash, skipping update");
      return;
    }
    updateUpdateState({
      updateInProgress: true,
    ...
    logger2?.error?.("[update] update task failed", {
...
var EnrollmentStore = class {
  ...
  normalizeCapabilities(state) {
    ...
    const changed = normalized.length !== capabilities.length || normalized.some((value, index) => value !== capabilities[index]);
    if (changed) {
  ...
  load() {
    if (!import_fs2.default.existsSync(this.statePath)) return null;
    try {
      const raw = import_fs2.default.readFileSync(this.statePath, "utf8");
  ...
  getPaths() {
    ...
    return {
      ...
      clientCert: import_path2.default.join(dir, "mtls-client.crt.pem"),
      clientKey: import_path2.default.join(dir, "mtls-client.key.pem"),
      caBundle: import_path2.default.join(dir, "mtls-ca.pem")
    };
  }
};
...
var config = {
  ...
  agentVersion: process.env.AGENT_VERSION || "1.1.3",
  coreVersion: process.env.CORE_VERSION || "1.1.3",
...
function getWindowsDeviceId() {
  ...
  try {
    const query = (0, import_child_process2.execSync)(`reg query "${regPath}" /v DeviceId`, { encoding: "utf8" });
...
var UNIX_BASE_DIR = "/var/lib/tracenium";
async function writeWindowsMetadata(meta) {
  const regPath = WINDOWS_REG_PATH;
  function runReg(args) {
    const result2 = (0, import_child_process3.spawnSync)("reg", args, { stdio: "ignore" });
    if (result2.status !== 0) {
...
async function retry(fn, attempts = 5, baseDelay = 1e3) {
  let lastError;
...
async function waitForPrivSvcPipe(timeoutMs = 2e4) {
...
async function generateCsrViaPrivSvc() {
  const net4 = await import("net");
...
async function installCertViaPrivSvc(clientCertPem, caBundlePem) {
  const net4 = await import("net");
  const deviceId = getDeviceId();
  const tenantId = "bootstrap";
  const request = JSON.stringify({
    ...
    client.on("data", (data) => {
      ...
      while ((idx = response.indexOf("\n")) !== -1) {
        ...
        try {
          ...
          client.destroy();
          resolve({
            clientCertThumbprint: String(result2.clientCertThumbprint || ""),
            issuingCaThumbprint: result2.issuingCaThumbprint ? String(result2.issuingCaThumbprint) : void 0
          });
          return;
        } catch (err) {
          reject(err);
...
function buildLocalEnrollmentState(store) {
  ...
  const state = {
    tenantId,
    deviceId,
    enrolledAtUtc: (/* @__PURE__ */ new Date()).toISOString(),
    mtls: {
      ...
      const res = await retry(
        async () => {
          ...
          if (!response.ok) {
            ...
            if (response.status === 401 || response.status === 403 && txt.includes("Token expired")) {
...
function createPrivSvcClient() {
  switch (process.platform) {
    case "win32": {
      const { PrivSvcClient: PrivSvcClient4 } = (init_privsvc_client_windows(), __toCommonJS(privsvc_client_windows_exports));
      return new PrivSvcClient4();
    }
    case "darwin": {
      const { PrivSvcClient: PrivSvcClient4 } = (init_privsvc_client_macos(), __toCommonJS(privsvc_client_macos_exports));
      return new PrivSvcClient4();
    }
    case "linux": {
      const { PrivSvcClient: PrivSvcClient4 } = (init_privsvc_client_linux(), __toCommonJS(privsvc_client_linux_exports));
      return new PrivSvcClient4();
    }
    default:
...
var PolicyStore = class {
  ...
  async save(policyVersion, policyHash, policyJson) {
    ...
    const record = {
      ...
      policyHash,
      policy: policyJson,
      appliedAtUtc: (/* @__PURE__ */ new Date()).toISOString()
    };
...
var PolicyRuntime = class extends import_events4.EventEmitter {
  constructor(store, logger2) {
    super();
    this.store = store;
    this.logger = logger2;
  }
  ...
  isModuleEnabled(module2) {
    return this.policy.modules?.[module2] ?? false;
  }
  isInventoryEnabled() {
...
var PluginManager = class {
  ...
  async registerBuiltinPlugins() {
    try {
      const { collectAMP: collectAMP2 } = await Promise.resolve().then(() => (init_amp(), amp_exports));
      this.register({
        name: "amp",
        tasks: {
          collect: async (ctx) => collectAMP2(ctx)
        }
      });
    } catch (err) {
...
var Scheduler = class {
  ...
  async start(ctx) {
    ...
    this.addPolicyListener(ctx, "inventoryIntervalChanged", (interval) => {
  ...
  async runInventory(ctx) {
    ...
    try {
      ...
      const hasAnyChanges = Object.values(namespaces).some((ns) => {
        if (!ns) return false;
...
var MAX_FACTS_IPC_BYTES = 64 * 1024;
var FACTS_CHUNK_SIZE = 48 * 1024;
...
function attachPrivPushHandler(ctx, onPush) {
  const priv = ctx.priv;
...
function createGrpcClient(ctx) {
  ...
  const stream = new import_events5.EventEmitter();
    ...
    writeChain = writeChain.then(async () => {
      ...
      if (msg?.heartbeat) {
        const deviceId = String(
          msg.heartbeat.deviceId || ctx.enrollment.deviceId || ""
        );
...
async function sendControlAck(ctx, eventId, status, message) {
  try {
    await ctx.priv.call({
      v: 1,
      id: `ack-${eventId}`,
...
function parseRunJobPayload(runJob) {
...
async function executeRunJob(ctx, runJob) {
  const jobId = String(runJob?.jobId || "").trim();
  const jobType = String(
    runJob?.jobType || runJob?.type || runJob?.task || ""
  ).trim();
  ...
  switch (jobType) {
    ...
    case "agent_update": {
      ...
      if (ctx._agentUpdateInProgress) {
        return {
          status: 1,
          message: "agent_update retry: update already in progress"
        };
      }
      ctx._agentUpdateInProgress = true;
      try {
        await runUpdateTask(ctx, {
        ...
        ctx._agentUpdateInProgress = false;
      }
    }
    default:
      return {
...
function startGrpcStream(ctx) {
  ...
  const runningJobIds = /* @__PURE__ */ new Set();
  ...
  stream.on("data", (msg) => {
    ...
    if (msg.agentUpdate) {
      ...
      ctx._agentUpdateInProgress = true;
      const { runUpdateTask: runUpdateTask2 } = (init_update_task(), __toCommonJS(update_task_exports));
      setImmediate(() => {
        runUpdateTask2(ctx, {
          targetVersion: version,
          logger: ctx.logger
  ...
  const armRetryTimer = () => {
    if (stopped) return;
    const delayMs = outbox.getNextReadyDelayMs?.();
  ...
  const drainOnce = () => {
    ...
    try {
      ...
      draining = false;
      const nextDelay = outbox.getNextReadyDelayMs?.();
      if (nextDelay === 0) {
  ...
  startDelayTimer = setTimeout(() => {
    if (!stopped) {
      ...
      if (typeof outbox.onChanged === "function") {
        unsubscribeOutbox = outbox.onChanged(() => {
