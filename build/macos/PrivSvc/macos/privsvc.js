...
var require_tokenize = __commonJS({
  "node_modules/protobufjs/src/tokenize.js"(exports2, module2) {
    ...
    function tokenize(source, alternateCommentMode) {
      ...
      var offset = 0, length = source.length, line2 = 1, lastCommentLine = 0, comments = {};
      var stack = [];
      var stringDelim = null;
      function illegal(subject) {
        return Error("illegal " + subject + " (line " + line2 + ")");
      }
      function readString() {
      ...
      function setComment(start, end, isLeading) {
        ...
        comments[line2] = comment;
        lastCommentLine = line2;
      }
      ...
      function cmnt(trailingLine) {
        ...
        if (trailingLine === void 0) {
          comment = comments[line2 - 1];
          delete comments[line2 - 1];
...
var require_channelz = __commonJS({
  "node_modules/@grpc/grpc-js/build/src/channelz.js"(exports2) {
    ...
    function connectivityStateToMessage(state2) {
      switch (state2) {
  ...
  "node_modules/@grpc/grpc-js/build/src/load-balancer-round-robin.js"(exports2) {
    ...
    var RoundRobinLoadBalancer = class {
      ...
      countChildrenWithState(state2) {
        return this.children.filter((child) => child.getConnectivityState() === state2).length;
      }
...
var RUN_DIR = configuredSocketPath ? import_path.default.dirname(configuredSocketPath) : "/var/run/tracenium";
...
function handleControlMessage(msg) {
  if (msg.ack) {
    push("grpc.ack", {
      ...
      receivedAtUtc: msg.ack.receivedAtUtc || (/* @__PURE__ */ new Date()).toISOString(),
      serverTimeUtc: msg.ack.serverTimeUtc || void 0
    });
    return;
  }
  if (msg.runJob) {
...
async function handleGrpcConnect(req, pushSink) {
  try {
...
async function handleFactsChunk(req) {
  const params = req.params || {};
  ...
  try {
    ...
    await write({
      ...
      ack: {
        ...
        receivedAtUtc: req.params?.receivedAtUtc || (/* @__PURE__ */ new Date()).toISOString()
      }
    });
    return success(req.id, { accepted: true, eventId });
  } catch (err) {
...
function handleClient(socket) {
  ...
  socket.on("data", async (chunk) => {
    ...
    for (const raw of lines) {
      const line2 = raw.replace(/\r$/, "").trim();
      if (!line2) continue;
      let req = null;
      try {
        req = JSON.parse(line2);
      } catch {
      ...
      if (!req) continue;
      try {
        const resp = await routeRequest(req, push2);
